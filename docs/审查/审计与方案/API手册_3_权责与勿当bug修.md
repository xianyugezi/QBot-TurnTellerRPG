...
| **T9** | **审计3 §7 D8：`damage_base`/`heal_rate` 兼容保留键** | `qbot_rpg/content/field_meta.py:1440`/`:1444`（两个 `FieldMeta`）+ 批74 裁决注释 `:1426-1438`；Python 侧 `formula_loader` **不消费**；审计原文 `:1381-1391` 为**行号漂移**（批74 前为 `:1424-1436`） | ✅ **批74 裁定：保留**。查证：① JS 侧 `web/static/index.html` 零硬编码（表单按 `FieldMeta` 动态渲染）；② 8 个既有包 `formula.json` 实带两键；③ 测试正面锁定；④ 属兼容承诺。删除三条件见代码注释 / 手册 §六 |

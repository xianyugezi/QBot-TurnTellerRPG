...
## 【2026-08-27 M4 设计审查裁决（设计审查_批次2 P1-1/P1-2，用户拍板）】

1. **对话树深度恢复可配**（P1-1）：删除「≤2 层硬约束」，恢复 `settings.max_dialog_depth`（默认 2，0=不限）——深度阈值取配置，超深软拦。D02/深度约束/TC-03 同步。
2. **发牌员策略枚举保留细化版**（P1-2，用户拍板选细化）：保留 `rotate/random/condition` 三策略（定稿 first_match/weighted/random 作兼容映射：first_match→condition、weighted→random 等权、random→random），登记为细化裁决并保留兼容迁移提示。
（审查报告 审查_M4设计_批次2_jspace.md P1-1/P1-2）

...
class BattleEngine:
    ...
    def _owned_effect_ids(self, side: str) -> Optional[List[str]]:
        ...
        - 数据源 = 快照 combatant 的 `OWNED_EFFECT_IDS_KEY`（装配层展开的装备被动
          / 效果归属——见 `core/equip_mods.worn_passive_effect_ids`）；
        - **键缺省 / 值畸形 → None**（= 不启用归属过滤，分派器全库扫描 =
          本机制引入前的旧行为，逐字段一致）；
        - 键存在（含空列表）→ 返回 id 列表（空集 = 本侧不拥有任何效果 → 零候选）。
        """
        comb = self._snap.get(side)
        raw = comb.get(OWNED_EFFECT_IDS_KEY) if isinstance(comb, Mapping) else None
        if raw is None:
            return None

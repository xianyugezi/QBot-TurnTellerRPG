...
**"提示不拦截"原则（本框架明文设计）**：黄提示只进 `warnings`，`ValidationReport.ok = not errors`（`models.py:57`/`:89`）——**只有红拦阻断**。⚠️ 别把"未知枚举"一律当黄：`_check_value` 的 **enum 分支是红拦 R-1**（`validator.py:3656`）；只有**开放清单**里的"未登记时点 / 轴"才走黄提示（Y-19 / Y-18）。编辑期还有"**产品明确允许**"的容忍：删/改名条目的悬空 R-4（`editor_ops.py:948-961`）、空骨架模块先启用后填写（`:1093-1106`）→ **转黄提示，不阻断落盘**。

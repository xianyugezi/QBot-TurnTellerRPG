...
class BattleEngine:
    ...
    def _on_battle_time_advance(self, ev: Mapping[str, Any]) -> None:
        ...
        # 合并适配（cloudsea-pack）：环境场周期器（九期207·G1B）与九态积蓄衰减
        # （九期211）原挂旧回合制 end_turn 的「整轮收尾」；CTB 删除该集中点后，
        # 二者同属**全局（非 actor）时间效果** → 迁入本位点（与换季同拍）。
        # 二者键缺失/未装配 → 零操作（opt-in，见各方法注）。
        # TODO merge-adapt：field_periods 的「每 N 回合」计数基准由旧 turn（整轮
        # +1）变为 action_seq 镜像（每行动 +1），content 侧 every 数值口径需按
        # 新时间轴重标定。
        self._tick_field_periods()
        self._tick_cloudsea_ailments()
        # 合并适配（cloudsea-pack）：云海 212 回合边界资源轴 tick（opt-in：
        # tick_per_round/tick_floor/on_full，见 ResourceLifecycle.settle_round_tick）
        # 同属全局时间效果 → 同位接入。TODO merge-adapt：frozen_sides（被控
        # 保留集合）与 on_full proc 消费属装配层职责，CTB 装配口径未定前传
        # None（缺省零行为）；返回的 on_full_fired 暂不消费。
        from qbot_rpg.core.resource_lifecycle import (  # noqa: PLC0415
            ResourceLifecycle,
        )
        ResourceLifecycle(self._resource_registry).settle_round_tick(self._snap)
    ...
    def _start_actor_turn(self, actor: str) -> bool:
        ...
        # 合并适配（cloudsea-pack）：本调用为云海 opt-in 附加，挂在旧回合制
        # `_start_actor_turn`（每 actor 行动头即派发）；CTB 下维持同位点（每
        # actor 行动头派发一次）。TODO merge-adapt：旧 1g1b 下 `_start_actor_turn`
        # 每轮仅进一次 → round_head≈整轮一次；CTB 下每 actor 行动头各派一次，
        # 「整轮一次」语义是否需要收敛到 BATTLE_TIME_ADVANCE 位点待核。
            ...
            if str(action.get("air_drop") or "").strip().lower() == "knockdown":
                ...
                if self._try_air_recover(action, insts):
                    ...
                    execute_action({"type": "reposition", "target": "self",
                                    "height": "ground"}, _ctx_r, _rt_r)
                    self._absorb_runtime(_rt_r)
                    self._snap.setdefault("battle_notes", []).append(
                        {"type": "air_recover", "side": "player", "attacker": "enemy"})
                    _ev_rc: Mapping[str, Any] = {"type": "air_recover",
                                                 "actor": "player", "attacker": "enemy"}
                    if isinstance(effects, list):
                        effects.append(_ev_rc)
                    return _ev_rc

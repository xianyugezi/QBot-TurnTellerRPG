...
> ⚠️ **本文档已被《编辑器重写_需求与约束》取代（2026-09-16）；附录 B 的 9 项【待定】随之作废（如需追溯见 git 历史）**。
> 取代依据（批30 核查）：① 本文档援引的权威契约 `docs/细化/细化_5a_编辑器契约.md` / `docs/细化/细化_5a2_编辑器扩展页.md`
> 与 `docs/m12_编辑器摸底.md` 已于 2026-09-13 随旧编辑器删除（`docs/消息模板重构/02_遗留登记.md` #86）；
> ② 重写版编辑器以 `docs/编辑器重写_需求与约束.md`（需求侧唯一来源）+ `docs/编辑器重写_实现方案.md`（实施侧唯一来源）
> 为准，界面/API/字段元数据机制全部重定义；③ GM 指令权限系统由仍在仓的 `docs/细化/细化_5b_GM指令契约.md` 承载。
> 本档仅作历史追溯，不再具有约束力。

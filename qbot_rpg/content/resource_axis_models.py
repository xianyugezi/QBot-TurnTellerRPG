...
# 云海九期（cloudsea-pack）212 扩展字段缺省（opt-in：缺省零行为）
...
class ResourceAxisDef:
    ...
    def tick_per_round(self) -> int:
        ...
        云海九期（cloudsea-pack）批次 212「tick 自然增长」缺口承载：云海轴注册段
        按需携带，ResourceLifecycle.tick_round_end 按本值应用（clamp [tick_floor, max]）。
        契约依据：云海九期（cloudsea-pack）排期 §零.6（勘察 ≈21 轴三缺口之一）。
        """
    ...
    def tick_floor(self) -> int:
        """tick 下限（衰减不破此值；缺省 0）。云海九期（cloudsea-pack）212 opt-in 字段。"""
    ...
    def on_full(self) -> str:
        ...
        云海九期（cloudsea-pack）批次 212「满槽 on_full」缺口承载：tick/apply_gain 后
        达到 effective max 时收集 (side, axis_id, proc_id) 事件，由装配层按
        M13 season_events/proc 同款消费；本引擎只收集不执行（零 engine 交叉 import）。
        """
...
def resource_axis_fields() -> Dict[str, FieldMeta]:
    """M1 注册段 10 键＋云海九期（cloudsea-pack）212 扩展 3 键 FieldMeta 登记表
    （stats.json 子条目 children 用）。"""
    return {
        ...
        # 云海九期（cloudsea-pack）212 扩展（opt-in）

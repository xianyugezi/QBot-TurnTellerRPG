...
- emoji 纪律：装饰性禁用（✅/❌ 功能性保留）；数据型功能图标豁免（物品 icon/NPC 类型图标/recommended_newbie 角标/GM 结果前缀）——**【2026-08-27 M5 裁决作废】用户拍板「不用 emoji」：除 ✅/❌ 功能性标记 + 排版符号（| → × / 「」【】）外，渲染输出不含任何 emoji（含数据型功能图标一律降级纯文本）；以 m5_shared_contract §4.1 为准**

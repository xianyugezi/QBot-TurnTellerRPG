...
| D-04 | 事件推送**逐事件类型独立开关 + 独立频道**（level_up/rare_drop/player_join/announcement），全走统一发送出口并受单群每分钟 ≤20 条限频；推送消息不占战斗结算消息配额 | 定稿 L1101 仅给两个类型与「默认关」，L1125/L1128/L1168 给出公告/欢迎/预警三类系统投递且复用公告通道 → 收敛为事件类型注册表；【规则】L499-503 频率约束与 3e2 D-06 同口径【补白】 | TC-10~TC-12 |
...
|---|---|---|---|---|
| 世界 | `level_cap` | 等级上限/满级经验去向 | 3b/4f RUL-11 | 【框架】L295-296 |
...
依据：【框架】L295「内容包配置【等级上限】（如 45 级）；满级后经验不再增长（或经验转货币，可选）」/ L296「升级时 HP/MP 回满」。
...
依据：【框架】L1104-1109（enabled 默认 true / format 默认 `Lv[等级].[玩家名] -[称号]-` / 占位符 / 显示位置首行 / 只建议不限制）；3d §1.5 + 附 L332（7 字段消费语义已细化，**本契约不重复造字段，仅登记落点**）：

```jsonc
"message_prefix": {
  "enabled": true, "format": "Lv[等级].[玩家名] -[称号]-",
  "show_on_system": false, "per_channel": "all",
...
- 校验：红黄规则按 3d 附 L332 + 【前缀】§九（【实现层规划】L3327「前缀段红黄按【前缀】§九」）→ 本契约 V9~V12 不重复覆盖。
...
- **限频硬约束**：全部事件推送走统一发送出口，单群每分钟 ≤20 条（【规则】L499-503，3e2 D-06 同口径）；推送消息不计入战斗结算消息配额；同事件（升级/掉落）多次触发合并为一条（同回合多掉落合并为「获得 2 件稀有物品」）。

...
p3 = run("_gen_templates_part3.py")
...
## 四、待接线功能（白名单已有 / 框架规划，指令壳未实现 → 无消息模板可采集）

以下功能在 parsers 白名单/帮助菜单中已登记，但**指令壳尚未接线**（批次7 装配 / 后续里程碑 / 内容包扩展），
消息模板待对应功能实现后补充：

| 功能 | 状态 | 说明 |
|---|---|---|
| /使用 物品 | 待接线 | 使用背包物品（CakeGame 模板 11 参考） |
| /锁定怪物 | 待接线 | 锁定当前地图怪物目标 |
| /状态 | 待接线 | panel_render 渲染助手已存在，指令未注册（批次7） |
| /怪物 /地图 | 待接线 | 怪物/地图列表查询 |
| /对话 | 待接线 | NPC 对话（对话壳 DELAYED） |
| /采集 /合成 /炼金 /锻造 /强化 /调合 /镶嵌 /拆珠 | 待接线 | 制造生活系统（M6/内容包） |
| /快捷绑定 /快捷解绑 /快捷列表 | 机制级已承载 | 绑定校验+路由展开有，指令 handler 未注册（批次7） |
| /转职 /排行 /队伍 /公会 /PVP | 内容包候选 | 框架规划（参考 CakeGame），未实现 |
...
| BREP-24 | `战斗结束：{胜/负/平}｜行动数 {N}｜输入 /战斗记录 查看明细` | 结束汇总（win 无此行） |

...
批54 · 特效预设（**同一机制的第二来源**，不新造一套）
-----------------------------------------------------
`content/effect_presets.py` 声明设计 `特效整理设计_2_框架预设包.md` §1 的**特效预设族**
（`equipment` 模块；同族两方向 = 两条预设）。它经 `entry_presets_for()` **投影成上表同形
条目**，由本模块的 `framework_presets(module)` 一并返回 → 走**同一**合并（框架默认 ∪ 包
声明 − 关闭）/ 同一编辑器链路。`items` 模块的六种通用品类**逐字段不变**（批17/18 契约）。

...
依据：docs/细化/细化_6c_资源轴与职业机制.md：
  ...
  - §2.2 EFF-2（进战懒加载）/ EFF-5（行动校验唯一入口：技能 ∈ 当前季节组
    ∪ 通用组；不在 → 拒绝并提示「此术式与当前时节不合」；引用在行动开始
    懒重读时更新）；
...
覆盖矩阵（17 用例 = A 解析 5 + B 判定 7 + C 行动校验 3 + D 引擎 2）：
  ...
  C 行动校验（EFF-5 唯一入口）：非当季 → 被拒不消耗行动（结构化判定 +
...
# ===========================================================================
# C 行动校验（EFF-5 唯一入口：非当季 → 被拒不消耗行动）
# ===========================================================================
def test_validate_out_of_season_rejected_no_turn() -> None:
    """非当季 → 被拒不消耗行动（TC-09：春季选夏季技能 → 行动校验拒绝）。

    复用 rejected 管道语义：不消耗行动、连段不变、可反复尝试——本引擎只出

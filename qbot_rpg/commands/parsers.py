...
DEFAULT_WHITELIST = frozenset(
    {
        ...
        "攻击", "使用", "进入", "锁定怪物", "采集",
        # W02 制造可快捷（全局模式下）
        "合成", "炼金", "锻造", "打造", "铸造", "强化", "调合", "镶嵌", "拆珠",
        # 批41 · 深度打造主入口（决策记录 §5.1：选定名 `精造`，零冲突）——独立指令名
        # 注册须进白名单才能被 S5 前缀匹配触发（P-04/P-05 同款裁决；缺白名单静默不响应）。
        "精造",
        # 批62 · 装备淬炼独立指令壳（批57 引擎/状态/配置/校验之上补指令；四源扫描零冲突）
        # ——独立指令名须进白名单（P-05 同款裁决；缺白名单静默不响应）。
        "淬炼",
        # M12.5 强化（2026-09-06）：/强化信息 /强化保护 独立指令名须进白名单才能被
        # S5 前缀匹配触发（P-04/P-05 同款裁决；缺白名单静默不响应——M9 锻造先例）
        ...
        # 独立指令名注册须进白名单才能被 S5 前缀匹配触发（P-05 同款裁决；缺白名单静默不响应）
        "套装", "客制",
        # M12.5 强化查询/保护（2c3b §5.2 指令参数表：/强化信息 /强化保护 独立注册；
        # P-04 最长前缀匹配防 /强化 吞前缀——白名单登记缺一不可）
        "强化信息", "强化保护",
        # M12.5 木桩训练（怪物模块 §十五）：/木桩 /调整木桩 独立指令名须进白名单
        # 才能被 S5 前缀匹配触发（P-05 同款裁决；缺白名单静默不响应）
        "木桩", "调整木桩",
        # M12.5 查看目标（框架 7.6 L1356/L1367）：/查看目标 战斗指令独立注册——
        # 须进白名单才能被 S5 前缀匹配触发（缺白名单静默不响应）
        "查看目标",
        # M12.5 赠送（定稿 L1286/L953）：/赠送 <物品>*<数量> <玩家>——独立指令名
        # 须进白名单才能被 S5 前缀匹配触发（缺白名单静默不响应）
        "赠送",
        # M12.5 温室（2c5c FARM-07/08）：/温室 独立指令名须进白名单（P-05 同款裁决）
        "温室",
        # M12.5 委托（2c5b CMD-01/02：委托板独立指令名，双板仲裁带板前缀）
        "委托",
        # M12.5 品评会（2c5c CT-01/05：/投稿 /排行榜 独立指令名须进白名单）
        "投稿", "排行榜",
        # W03 状态查询
        "状态", "背包", "背包筛选", "怪物", "地图", "位置", "我的技能", "我的状态",
        "时间", "天气",  # 2026-09-06 时间/天气（explore 组）
        # 基础指令组（4f 常见）
        "注册", "注销", "角色", "角色详细", "装备", "卸下",
        "技能", "技能详情", "技能派生", "帮助", "休息", "对话",
        "任务", "任务信息", "接取", "交付", "商店", "商店列表", "商店进入", "购买", "出售", "签到",
        # 3f 单机向体验（F-05/F-06/F-07）：/调查 目标可选（交互点别名/地图名/省略=当前地图）
        "调查",
        # 3f 单机向体验（F-11/F-12）：/图鉴 分册总览/分册分页（未收集条目 ???）
        "图鉴",
        # 2026-08-30 实机反馈：帮助/白名单引导过但尚未实装的指令 → stub 提示（防静默空回）
        "职业", "职业列表", "职业详情", "转职", "锁定",
        # 快捷绑定指令（规范 6.6）
        "快捷绑定", "快捷解绑", "快捷列表",
        ...
        # ——独立指令名缺白名单 → S5 静默不响应（P-05 同款裁决，M9 锻造先例）
        "钓鱼", "鱼讯", "收杆",
        # M11 成就（4c §七）：/成就 /成就信息 /称号 三指令
        "成就", "成就信息", "称号",
        # M11 PVP（4e CMD-05/06，定稿 L1287-1288）：/锁定玩家 /攻击玩家 双原语
        "锁定玩家", "攻击玩家",
        # GM 指令（5b；强制 / 前缀，永不快捷，规范 L128）；批30（2026-09-16）删 G13「编辑」
        "重载", "封禁", "日志", "设置",
        # M12 批3 路3A（5b G2/G3/G4/G12）：/备份 /恢复 /存档导出 /封禁列表
        "备份", "恢复", "存档导出", "封禁列表",
        # 批75 · GM 运维（5b G5/G6/G7/G9/G11）：/调试 /测试 /广播 /玩家查询 /解封
        "调试", "测试", "广播", "玩家查询", "解封",
    }
)
...
DEFAULT_FREE_ARG_COMMANDS = frozenset({"背包筛选",
                                        # M12.5 委托（2c5b CMD-05：/委托 交付 <委托> <道具>
                                        # 三位置参数——自由参数豁免 S7 铁律 3）
                                        "委托",
                                        # 批41 · 深度打造：/精造 <图纸> <材料>*<数量>,…
                                        # （多材料列表/多位置参数——自由参数豁免 S7 铁律 3）
                                        "精造",
                                        # 批62 · 装备淬炼：/淬炼 <装备> <属性>*<点数>[,…]
                                        # （多位置参数/`,` 列表——自由参数豁免 S7 铁律 3）
                                        "淬炼",
                                        # 批75 · GM 运维：/广播 <消息>（消息可含空格，
                                        # 多词按位置参数直取，处理器再 join——豁免 ≤2 上限）
                                        "广播"})

# GM 指令（5b L160 长清单：重载/封禁/日志/设置；批30 删 G13「编辑」→ 8 条含 M12 扩展；
# L128 强制 / 前缀 + 永不快捷 + 快捷禁绑 C02）。与 router spec.is_gm 对齐（P1-2c 对拍）：
# GM 判定用本集合而非 prefix_required（后者另含非 GM 的 /对话）；GM 要求 '/' 前缀
# **真正落在当前展开串上**，快捷展开的触发 '/' 不豁免（对齐 router._trigger_allowed）。
DEFAULT_GM_COMMANDS = frozenset({"重载", "封禁", "日志", "设置",
                                 "备份", "恢复", "存档导出", "封禁列表",
                                 # 批75 · GM 运维（G5/G6/G7/G9/G11）逐条接入
                                 "调试", "测试", "广播", "玩家查询", "解封"})

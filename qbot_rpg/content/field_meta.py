...
SETTINGS_CHILD_LABELS: Dict[str, Any] = {
    ...
    "battle": {
        "stun_enabled": "启用气绝", "stun_base_threshold": "气绝基础阈值",
        ...
        "stun_back_mult": "气绝反制倍率", "stun_decay_per_action": "每行动气绝衰减",
        "stun_escalation": "气绝递增", "stun_hint_at": "气绝提示阈值",
        "stun_ko_skip": "气绝跳过回合", "stun_ko_window": "气绝窗口",
        "stun_side_mult": "方位气绝倍率",
        "backstab_bonus": "背击加成", "crit_cond_low_hp": "逆境会心阈值",
    },
    "command_aliases": {},
    "contest": {
        "enabled": "启用",
...
# ---- settings 各段嵌套说明（批4.6 补：战斗/任务板/炼金/锻造/钓鱼/助手/竞技/行动条/经济）----
# 口径与 *_CHILD_HELP 一致：str 或 {_help, 子键…}；只作用于 SETTINGS_CHILD_LABELS 已声明的键，
# 不新增字段节点；依据 = qbot_rpg/core/battle.py `_BATTLE_DEFAULT_CONFIG` /
# battle_config.py 白名单区间 / docs/m8_contract_数据与校验 settings.alchemy 表 /
# forge_settings.py / fishing_models.py / docs/实现层规划文档 §3.1 quest_board /
# docs/审查参考/*定稿；无依据处不写。
SETTINGS_CHILD_HELP: Dict[str, Any] = {
    "currencies": {
        "id": "货币标识（存档与引用用它，如 coins/diamond）。",
        "name": "货币显示名。",
        "icon": "货币图标标识。",
        "cap": "该货币的持有上限。",
        "note": "备注（仅展示）。",
    ...
    "death_penalty": {
        "_help": "死亡惩罚：虚弱时长与三类掉落比例。",
        "weak_duration_sec": "死亡后虚弱状态的持续秒数。",
        "drop_currency": {"_help": "死亡掉落的货币规则。", "currency": "掉落的货币标识。",
                          "ratio": "掉落比例（0~1，1 = 全部掉落）。"},
        "drop_exp": {"_help": "死亡掉落的经验规则。", "enabled": "是否启用经验掉落。",
                     "percent": "掉落百分比。"},
        "drop_items": {"_help": "死亡掉落的物品规则。", "enabled": "是否启用物品掉落。",
                       "count": "掉落物品件数。"},
    },
    "slot_defs": {
        "_help": "装备部位定义表（键为部位标识）。",
        "name": "部位显示名。",
        "max": "该部位最大可装备件数。",
        "occupies": "装备该部位时同时占用的其他部位。",
    },
    "alchemy": {
        "_help": "炼金全局参数：模式/品质/精力/宝石与战斗调合。",
        "mode": "炼金模式：full 完整 / simple 简化 / off 关闭。",
        "quality_tiers": "品质档位区间表：档位 → [下限, 上限]，须单调且共同覆盖 0~100。",
        "quality_coef": "品质系数：各档位对应的产出倍率（>0）。",
        "chain_map": "连锁映射：连锁数 → 1~6 的档位整数。",
        "pp_cost": {"_help": "调合点数消耗。", "normal": "普通调合消耗点数。",
                    "super": "超级调合消耗点数。"},
        "pp_refresh": "点数刷新时机（如「会话重置」）。",
        "energy_enabled": "是否启用炼金精力条。",
        "energy_max": "精力上限（按职业档位配置）。",
        "energy_regen_sec": "精力恢复间隔秒数。",
        "energy_regen_sec_safe": "安全区内精力恢复间隔秒数。",
        "decompose_rate": "分解回收率（按品质/档位配置）。",
        "catalyst_unlock_tier": "触媒解锁所需职业档位。",
        "catalyst_consume": "合成时是否消耗触媒。",
        "gem.分解": "宝石分解回收的宝石数量。",
        "gem.复制": "宝石复制消耗系数（可小数）。",
        "gem.成品合成": "成品合成消耗的宝石数。",
        "gem.配方合成": "配方合成消耗的宝石数。",
        ...
        "gem.珠升阶": "珠升阶消耗的宝石数。",
        "gem.复制额外": "复制额外追加的消耗。",
        "gem_diminish": {"_help": "珠同名递减规则表。", "n": "同名次数阈值（≥2）。",
                         "mult": "该次数起的递减倍率（0~1）。"},
        "synth_exp": "合成经验公式（如「配方等级×1」）。",
        "sp_per_level": "每级获得的技能点。",
        "sp_panel": "技能面板条目表。",
        "战斗道具": {"_help": "战斗道具强度规则。", "强度公式": "战斗道具强度公式。",
                     "珠触发上限": "珠触发次数上限（≥1）。"},
        "战斗即时调合": {"_help": "战斗即时调合规则。", "auto_use": "是否自动使用。",
                         "per_battle_limit": "每场使用上限（≥1）。",
                         "proficiency_multiplier": {"_help": "熟练度乘区。",
                                                   "min": "乘区下限。", "max": "乘区上限。",
                                                   "curve": "乘区曲线形态。"}},
        "max_qty": "单次合成数量上限。",
        "job_tier_map": "职业档位映射表。",
        "farming": {"_help": "种植规则。", "enabled": "是否启用种植。",
                    "harvest_sec": "作物成熟所需秒数。", "plots_max": "最大田块数。",
                    "trait_inherit": "特性继承配置。",
                    "greenhouse": {"_help": "温室规则。",
                                   "copy_cost": {"_help": "复制消耗。", "coins": "金币消耗。",
                                                 "gem": "宝石消耗。"},
                                   "copy_slot": "复制占用槽位。", "unlock_tier": "温室解锁档位。"}},
    },
    "forge": {
        "_help": "锻造全局参数。",
        "forge_fee": "锻造费用（金币整数或按节点等级计算的公式文本）。",
        "synth_ratio_3to1": "三合一合成比例。",
        "straight_forge": "是否允许直接锻造（不走上游节点）。",
        "decompose_rate": "分解回收率。",
        "exp_per_forge": "每次锻造获得的经验（整数或公式文本）。",
        "sets_enabled": "是否启用套装效果。",
        "augments_enabled": "是否启用客制强化。",
        "set_piece_counts": "各套装的部件数要求。",
        "set_tier_exact": "套装档位是否要求精确匹配。",
    },
    "fishing": {
        "_help": "钓鱼全局参数。",
        "mode": "钓鱼模式（开启/简化/关闭三态）。",
        "bait_ids": "可用鱼饵引用列表。",
        "daily_limit": "每日钓鱼次数上限。",
        "wait_sec": {"_help": "等待时间区间（秒）。", "min": "最短等待秒数。",
                     "max": "最长等待秒数。"},
        "energy": {"_help": "钓鱼精力配置。", "enabled": "是否消耗精力。"},
        "bait_bonus": {"_help": "鱼饵加成。", "gold": "金色鱼饵加成倍率。",
                       "rare": "稀有鱼饵加成倍率。"},
        "rod_full_bonus": {"_help": "满竿加成。", "gold": "金色满竿加成。",
                           "rare": "稀有满竿加成。"},
        "crown_thresholds": {"_help": "冠级尺寸阈值。", "reverse": "倒扣阈值。",
                             "silver": "银冠阈值。", "gold": "金冠阈值。"},
        "king_event": {"_help": "鱼王事件窗口。", "enabled": "是否启用鱼王事件。",
                       "window_daily": "每日出现窗口。", "chance": "出现概率。"},
    },
    "assistant": {
        "_help": "助手段配置。",
        "enabled": "是否启用助手。",
        "energy_items": "恢复精力的道具引用列表。",
        "unlock_tier": "助手解锁所需职业档位。",
        "helpers": "助手列表（每条含采集/品质/特性加成）。",
        "queue": {"_help": "助手队列配置。", "max_slots": "队列最大槽位。",
                  "tick_sec": "队列结算间隔秒数。"},
    },
    "battle": {
        "_help": "战斗数值参数（缺省回落引擎默认；键值语义见 core/battle.py）。",
        "stun_enabled": "是否启用气绝 KO 机制。",
        "stun_base_threshold": "气绝基础阈值（敌 resistance.stun 为百分比加成）。",
        "stun_escalation": "每次 KO 后阈值的倍率递增（防无限控场）。",
        "stun_decay_per_action": "每次行动收尾的气绝衰减比例（0 = 不衰减）。",
        "stun_ko_window": "KO 倒地窗口（持有者行动次数）。",
        "stun_ko_skip": "KO 是否吞掉怪物下一次行动（起身演出）。",
        "stun_hint_at": "气绝软提示阈值（进度比例；0 = 关闭）。",
        "stun_side_mult": "侧位命中积累倍率（0 = 仅正方位）。",
        "stun_back_mult": "背位命中积累倍率（0 = 不积累）。",
        "roar_light_delay": "轻咆哮的行动条后推量。",
        "roar_heavy_delay": "大咆哮的行动条后推量。",
        "roar_combo_clear": "咆哮是否震散玩家在途连势。",
        "rage_per_damage": "每 1 点伤害积累的怒气值。",
        "rage_cool_actions": "怒态持续的行动次数。",
        "stamina_max": "敌人耐力上限（一次 BOSS 战约 1~2 次疲劳）。",
        "stamina_drain_blunt": "玩家打击命中一次削减的耐力。",
        "stamina_regen_per_action": "每次行动边界恢复的耐力。",
        "enrage_damage_mult": "怒态敌方伤害倍率（>1 增伤）。",
        "enrage_recovery_mult": "怒态敌方行动恢复倍率（<1 提速）。",
        "fatigue_recovery_mult": "疲劳敌方行动恢复倍率（>1 拖慢）。",
        "fatigue_stagger_chance": "疲劳敌方行动自摔概率（0~1，0 = 关闭）。",
        "backstab_bonus": "背击加成比例（0~1，0 = 关闭）。",
        "crit_cond_low_hp": "「逆境」会心触发的血量比例阈值（0~1）。",
    },
    "contest": {
        "_help": "竞技（品评会）配置。",
        "enabled": "是否启用竞技。",
        "reward": {"_help": "竞技奖励。", "gem": "宝石奖励。", "title": "称号奖励。",
                   "reputation": {"_help": "声望奖励。", "submit": "提交奖励。",
                                  "win": "获胜奖励。"}},
        "score_weights": {"_help": "评分权重（各维度占比）。", "awaken": "觉醒维度权重。",
                          "potential": "潜力维度权重。", "quality": "品质维度权重。",
                          "trait": "特性维度权重。"},
    },
    "ctb": {
        "_help": "行动条（CTB）参数。",
        "enabled": "是否启用行动条时间制。",
        "default_recovery": "默认行动恢复值（基准速度）。",
    },
    "quest_board": {
        "_help": "任务板（委托板）配置。",
        "enabled": "是否启用任务板。",
        "refresh_days": "任务板刷新间隔天数。",
        "penalty": "超时未交付的声望惩罚比例。",
        "active_limit": "同时可接取的任务上限。",
        "daily_limit": "每日可接取任务上限。",
        "rep_levels": "声望等级门槛表（等级名 → 所需声望）。",
        "grade_bonus": {"_help": "各评级对应的奖励倍率。", "S": "S 级倍率。",
                        "A": "A 级倍率。", "B": "B 级倍率。"},
        "tiers": "任务板档位/委托条目表。",
    },
}

# ---- 批4.6 补：maps / quest / jobs / npc / shop 模块字段说明（顶层 + 嵌套）----
MAPS_FIELD_HELP: Dict[str, str] = {
    "id": "地图唯一标识（出口/复活点/副本入口用它引用）。",
    "name": "地图显示名。",
    "enemy_pool": "本地图可刷新的怪物引用列表。",
    "battle": "本地图是否触发战斗（false = 不遇敌）。",
    "revert": "回退标记（需要原地回退的地图段）。",
    "min": "战斗等级/难度区间下限。",
    "max": "战斗等级/难度区间上限。",
    "lower": "区间下限（与 min 同义，按内容形态择一）。",
    "upper": "区间上限（与 max 同义，按内容形态择一）。",
    "reset": "地图重置规则（刷新周期等）。",
    "safe_zone": "是否安全区（安全区内不触发战斗、精力恢复更快）。",
    "respawn_point": "死亡后复活指向的地图标识。",
    "desc": "地图说明文案。",
    "camp": "营地配置（名称/是否安全/解锁方式）。",
    "camp_name": "营地的显示名。",
    "npcs": "常驻 NPC 引用列表。",
    "monsters": "刷怪行列表（怪物引用/同场数量/刷新间隔）。",
    "exits": "四向通道出口（目标图/通行模式/解锁条件）。",
    "mechanics": "地图机制列表（如每步触发的伤害）。",
    "gate_guard": "门卫判定（未满足条件不放行）。",
    "gather_points": "采集点列表（物品/稀有度/概率）。",
    "dungeon_entrances": "副本入口列表（副本引用/名称）。",
}
MAPS_CHILD_HELP: Dict[str, Any] = {
    "monsters": {"_help": "刷怪行。", "enemy": "刷新的怪物引用。", "count": "同时在场上限。",
                 "respawn_minutes": "刷新间隔分钟数。", "name": "展示名。",
                 "hidden_boss": "是否为隐藏首领。", "intro": "出场台词。", "signal": "信号词。"},
    "exits": {d: {"_help": "%s向出口。" % zh, "to": "目标地图引用。", "mode": "通行模式。",
                  "condition": "解锁条件。"}
              for d, zh in (("up", "上"), ("down", "下"), ("left", "左"), ("right", "右"))},
    "mechanics": {"_help": "地图机制。", "id": "机制标识。", "desc": "机制说明。",
                  "on_step": {"_help": "每步触发。", "type": "触发类型。", "damage": "伤害值。"}},
    "gather_points": {"_help": "采集点。", "id": "采集点标识。", "item": "产出物品引用。",
                      "rarity": "稀有度。", "rate": "出现概率。"},
    "dungeon_entrances": {"_help": "副本入口。", "dungeon": "副本引用。", "name": "入口名称。"},
    "camp": {"_help": "营地。", "name": "营地名称。", "safe": "是否安全区。",
             "unlock": "解锁方式。"},
}
QUEST_FIELD_HELP: Dict[str, str] = {
    "id": "任务唯一标识。",
    "name": "任务显示名。",
    "desc": "任务说明文案。",
    "type": "展示标签（collect/deliver/slay/explore/intel 等）；判定由 conditions 驱动。",
    "main": "是否为主线任务（沿 unlock_chain 推进章节序号）。",
    "zone": "所属区域（用于任务分类与展示）。",
    "consume": "交付是否消耗物品（true = 交付扣物 / false = 计数）。",
    "repeatable": "是否可重复接取。",
    "conditions": "完成条件数组（统一条件引擎 {var,op,value,param}）。",
    "reward": "奖励条目数组（物品/货币/经验/声望；也支持内联键值串）。",
    "board": "任务板配置（槽位/刷新频率/上限）。",
    "timed": "限时修饰（截止时间，现实时间懒计算）。",
    "unlock_chain": "前置任务 ID（链式任务）。",
    "filter": "交付过滤（品质/特性/数量要求）。",
    "bonus": "条件倍率奖励（满足加分项时奖励 ×N）。",
    "npc": "任务发布/交付关联的 NPC。",
    "daily": "每日任务规则。",
}
QUEST_CHILD_HELP: Dict[str, Any] = {
    "conditions": {"_help": "完成条件行。", "var": "判定变量。", "op": "运算符。",
                   "value": "目标值。", "param": "附加参数。"},
    "npc": {"_help": "NPC 关联。", "id": "NPC 标识。"},
    "reward": {"_help": "奖励条目。", "coins": "金币。", "gem": "宝石。", "exp": "经验。",
               "rep": "声望。", "prof": "熟练度。", "item": "物品引用。", "count": "数量。"},
}
JOBS_FIELD_HELP: Dict[str, str] = {
    "id": "职业唯一标识（禁空格）。",
    "name": "职业显示名。",
    "difficulty": "上手难度建议：simple 简单 / advanced 进阶 / complex 复杂（仅建议不拦截）。",
    "playstyle": "玩法一句话（≤20 字）。",
    "recommended_newbie": "是否推荐新手（注册缺省职业取推荐标记）。",
    "resource_axes": "资源轴引用列表（取自 stats.json 注册表）。",
    "mechanic_tags": "机制标签（用于检索/展示）。",
    "weapon_types": "可用武器类型列表。",
    "growth": "九属性职业成长率（白值增量 = 白值×成长×职业成长率）。",
    "transform": "形态切换段（缺省 = 无形态切换职业）。",
    "description": "职业介绍文案。",
}
JOBS_CHILD_HELP: Dict[str, Any] = {
    "growth": {"str": "力量成长率。", "int": "智力成长率。", "con": "体质成长率。",
               "spr": "精神成长率。", "foc": "专注成长率。", "agi": "敏捷成长率。",
               "lck": "幸运成长率。", "hp": "生命成长率。", "mp": "法力成长率。"},
    "transform": {"_help": "形态切换。", "transform_skill": "触发技能 ID。",
                  "transform_to": "目标形态 ID。", "duration": "持续方式：turns 按回合 / battle 整场。",
                  "turns": "持续回合数（duration=turns 时）。", "revert": "结束后是否还原。",
                  "cooldown": "形态冷却回合数。", "dispel_reverts": "被驱散时是否还原。",
                  "skill_set": "形态技能组 ID。", "equip_restrict": "形态装备限制列表。",
                  "derive_chains": "形态专属派生链列表。",
                  "state_policy": {"_help": "切换时状态处理。", "combo": "连段保留或清零。",
                                   "marks": "印记保留或清零。", "buff": "增益保留或清零。"}},
}
NPC_FIELD_HELP: Dict[str, str] = {
    "id": "NPC 唯一标识。",
    "name": "NPC 显示名。",
    "icon": "NPC 图标标识。",
    "map": "地图挂点（所在地图与坐标）。",
    "type": "NPC 类型：merchant 商人 / blacksmith 铁匠 / quest 任务 / inn 旅店 /"
            " alchemist 炼金 / fisher 渔夫 / quest_giver 任务发布 / tutor 导师。",
    "desc": "NPC 说明文案。",
    "visible": "是否在场景中可见。",
    "dialogues": "对话配置（问候语与选项）。",
    "interactions": "交互动作列表（对话/商店/治疗/修理/传送/情报/教学等）。",
    "quests": "关联任务列表。",
    "dealer": "发牌员配置（轮转/随机/条件三策略）。",
    "repair": "修理配置（可修理的装备范围与费用）。",
    "tutorials": "教学条目列表。",
    "shop_refs": "关联商店引用列表。",
    "intel_refs": "情报引用列表。",
}
NPC_CHILD_HELP: Dict[str, Any] = {
    "dialogues": {"_help": "对话配置。", "greeting": "进入对话时的问候语。",
                  "options": {"_help": "对话选项。", "text": "选项文本。",
                              "next": "下一段对话。", "action": "选项动作。",
                              "quests": "可接取任务。", "shop_refs": "打开商店。",
                              "intel_refs": "查看情报。", "tutorials": "查看教学。",
                              "heal": "治疗。", "cost": "消耗。"}},
    "interactions": {"_help": "交互动作行。", "action": "动作类型。", "text": "展示文本。",
                     "repeat": "是否可重复。", "turns": "持续回合数。", "map": "地图挂点。",
                     "items": "涉及物品。", "effects": "附加效果。",
                     "quests": {"_help": "关联任务。", "quest_id": "任务 ID。"},
                     "condition": {"_help": "触发条件。", "var": "判定变量。", "op": "运算符。",
                                   "value": "目标值。", "param": "附加参数。"},
                     "cost": {"_help": "消耗。", "coins": "金币。"},
                     "heal": {"_help": "治疗。", "hp": "恢复生命。", "mp": "恢复法力。"}},
    "quests": {"_help": "关联任务。", "quest_id": "任务 ID。"},
}
SHOP_FIELD_HELP: Dict[str, str] = {
    "id": "商店唯一标识。",
    "name": "商店显示名。",
    "icon": "商店图标标识。",
    "type": "商店类型：normal 普通 / general 通用 / black 黑市 / reputation 声望 /"
            " event 活动 / quest 任务。",
    "currency": "默认结算货币标识。",
    "level_required": "进入/购买所需的等级门槛。",
    "reputation_required": "购买所需的声望门槛。",
    "open_condition": "开放条件（未满足时不出现在场景）。",
    "refresh": "刷新规则（模式/时间/间隔）。",
    "items": "商品列表（物品/价格/库存/限购/时段）。",
    "pool": "黑市池（黑市候选商品）。",
    "price_fluctuation": "价格波动配置。",
    "visible": "是否可见。",
    "desc": "商店说明文案。",
    "listing_count": "黑市每次上架的商品数。",
}
SHOP_CHILD_HELP: Dict[str, Any] = {
    "items": {"_help": "商品条目。", "sold_out_once": "售罄后是否永久下架。"},
    "refresh": {"_help": "刷新规则。", "mode": "刷新模式：daily 每日 / weekly 每周 /"
                " manual 手动 / fixed 固定。", "start": "开始时间。", "end": "结束时间。",
                "hour": "刷新小时。", "interval": "间隔天数。"},
    "pool": {"_help": "黑市池。"},
}
    ...
    return {
        ...
        "jobs": ModuleMeta(entry_type="list",
                           fields=_decorate_field_meta(jobs_fields, {}, JOBS_FIELD_LABELS,
                                                       JOBS_CHILD_LABELS, JOBS_FIELD_HELP,
                                                       JOBS_CHILD_HELP),
        ...
        "maps": ModuleMeta(entry_type="list",
                           fields=_decorate_field_meta(maps_fields, MAPS_FIELD_GROUPS,
                                                       ...
                                                       MAPS_FIELD_HELP, MAPS_CHILD_HELP),
        ...
        "quest": ModuleMeta(entry_type="list",
                            fields=_decorate_field_meta(QUEST_FIELDS, QUEST_FIELD_GROUPS,
                                                        ...
                                                        QUEST_FIELD_HELP, QUEST_CHILD_HELP),
...
__all__ = [
    ...
    # 批4.6 补：settings 各段嵌套说明 + maps/quest/jobs/npc/shop 模块说明

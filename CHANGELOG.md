...
- **批76（2026-09-21）**：**《矛盾与待裁决登记》按 26→28 条重核结果重写（纯文档）**。
  ...
  **销项 3 条**——**U9**（`/root/deliverables/U9_编辑器进阶能力_逐条核对.md` 769 行已交付，
  「是否需补专项逐条核对」的执行结果已完成）· **U10**（`qbot_rpg/content/field_meta.py:2855-2860`
  批13 C 已把刷怪行结构接到 maps 模块 → 收敛为「刷怪行字段」）· **X7**（脚本实测
  `content/*/templates.json` 表外旧键 **0**，门禁 `tests/unit/test_template_table_final.py:83-106`）。
  **补计 2 条**——**X17/X18**（批35 追加但未进 §二 汇总，独立条目 26 → **28 条**）。
  **纠正 2 条**——**U1**（删「目录条目标未实现」过期子句；`module_catalog.py:132-133` 已翻转可启用）·
  **X3**（目录条目描述 / `overlap_note` 与 `SLOTS_FIELD_DEFS` / `core/jewel` 珠插槽实现冲突，如实标 🟡）。
  **X13** 保持「已裁决 · 不做」（用户 2026-09-23 裁决，勿回退）。
  **§二 汇总同步 28 条现行状态**：✅ 已解决 / 已裁决 **23** / 🟡 部分解决 **3**（U4 / X3 / X6）/
  ❌ 仍未解决 **1**（X18）/ ⚪ 已失效 **0** / ⛔ 已裁决 · 不做 **1**（X13）；
  **X11 由批75 销项**（GM 5 已落地 → 真缺口清零），故 ✅ = 重核报告 ✅22 + X11 = 23、🟡 = 报告 🟡4 − X11 = 3。
  **元级教训写入维护注记**：登记表会漂移，**销项 / 改状态 / 补计后必须同步 §二 汇总**
  （本轮成因 = 批35 追加 X17/X18 未进汇总）；并记「本文下次对账基线 = 批76 HEAD」。
  **核对（未改动）**：`docs/指令清单_全量.md` 批75 的 G5/G6/G7/G9/G11 五行已为「已实现」，
  `:19` 汇总行已移除「时间天气」等过期表述（批75 声称的修正已落地）。页脚批次串仍为
...
- **批75（2026-09-23）**：**GM 运维 5 指令落地**（`docs/指令清单_全量.md` G 节 +
  ...
  **① `/调试` G5**（机主可下授）：`GmBackend.toggle_debug` 运行时开关（进程级，不落存档）+
  两态状态回显；模板 `gm_debug_status`。
  **② `/测试` G6**（机主可下授）：`GmBackend.smoke_test` **只读**（content JSON 语法 +
  路由连通，零写盘）+ 人话通过/失败结论（失败列首条）；模板 `gm_test_pass`/`gm_test_fail`。
  **③ `/广播` G7**（机主可下授）：≤200 字（`BROADCAST_MAX_CHARS`）即时广播全部群+私聊，
  复用 `ctx["announce"]` 公告通道（未装配降级，不硬造）；超长领域模板拦截。
  **`定时=HH:MM` 如实登记为缺口**：框架零 apscheduler + 解析器 token 集不含 `:`
  （`定时=20:00` 解析层即报未知分隔符）→ 只做即时广播；口径见 `docs/GM运维批75_实现口径.md`。
  **④ `/玩家查询` G9**（机主可下授）：只读脱敏摘要——QQ 中段打码（`mask_qq`，依据
  `开发规则文档:485`）+ 字段最小化（等级/货币/最近在线/封禁；无背包/收益/聊天）；
  未注册 → 错误模板（非静默）；审计 `target_qq` 存全号溯源。
  **⑤ `/解封` G11**（GM 默认授予集）：与 `/封禁`/`/封禁列表` 成对；不在名单 → 错误模板
  （含 `/封禁列表` 指引）；审计 E4。
  **接线面**：常量 `gm_constants.py`（G5/G6/G7/G9/G11）+ `_HANDLERS` + `GM_COMMANDS`
  13 条 + `parsers` 白名单/`DEFAULT_GM_COMMANDS` 双向一致（`check_consistency` 实跑为空）+
  `DEFAULT_FREE_ARG_COMMANDS` 补「广播」+ 模板表 16 新键；`scripts/verify/verify_m12.py`
  5b-TC-16/17/18/23/30 由 DELAYED 转 pytest 承载。
  **文档同步**：`指令清单_全量.md` 5 行转「已实现」+ 修正汇总行过期表述（时间天气/木桩/
  GM 运维已落地）；`矛盾与待裁决登记.md` X11 销项（汇总：已实现 3 / 待实测 2）；新增
  `docs/GM运维批75_实现口径.md`。页脚批次串 →「批75 · GM 运维指令」（`web/static/index.html`
  + 16 个批次串断言测试同步）。
  验收：全量 pytest **9067 passed / 0 failed / 10 skipped**；`ruff` 改动文件干净；
  `check_consistency` 通过（注册 ↔ 白名单双向一致）；换包验收 **12/12 PASS**；
  `git status --porcelain` 空。**未做/登记**：`/广播` 定时调度与 `群=` 限定（本批口径未要求）、
  `/测试` 三表 schema/引用完整性（归内容包校验器）、`GmBackend.ban_player`（G10 后端历史缺口，
  不动手）。
...
- **批69（2026-09-23）**：**新手上路收尾（走查 §3 左栏 / 中栏可见引导 + 推荐组合后的「空骨架」误拦）**。
  ...
  **① §3 左栏**：未启用行渲染**可见**的「点击启用」动作标签（`.tag.act` 强调色），不再只藏在
  hover `title` 里，并消除此前「未启用」重复两遍的冗余。**② §3 中栏**：0 条空态两分支（未启用任何
  模块 / 本模块无条目）各自给出下一步，与右栏口径一致（批65 已落地，本批核实并加回归）。
  **③ 新卡点（阻断）**：推荐组合会启用空骨架模块（如技能库无普攻 → V-7），修前这些红拦会把
  **其它**模块的新建 / 保存一并阻断。修法：条目级写路径（`validate_entry` / `save_entry` /
  `create_entry`）复用模块开关同一 `_tolerate_empty_modules` 机制——空骨架只提示不阻断，模块
  **非空但确有问题**照常红拦、零写入；容忍提示补模块中文名（不裸露英文键）。页脚批次串 →
  「批69 · 新手上路收尾」。回归：`tests/unit/test_editor_batch69_onboarding.py`。

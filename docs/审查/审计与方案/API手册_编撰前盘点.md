...
| **B5 门槛判定两口径并存（`tier_index_for_level` vs `level` 直比）** | 两条路径对“职业等级门槛”算法不同，看似“重复实现漂移” | 默认配置下两口径**等价**，属如实登记的并存（未收敛） | `记录.md:1159`；`docs/审查报告/审查_M8实现_批次E2_jspace.md:25`、`:103` | 中 | 贸然统一其中一条→非默认配置下职业门槛判定变化；须先查内容包实际配置 |

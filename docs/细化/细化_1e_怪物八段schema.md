...
> 版本：v1.1（2026-08-25 复审修订：P0-1 枚举归一闭环 / P1-1 行号漂移全量重刷〔定稿 396→398 行〕/ P1-2 概率语义对齐 AI 定稿 / 实现缺口补 special_actions+actions 字段 / 新增 §⑦ 跨文档登记）· 依据：《怪物模块设计定稿 v1.0.3》（下文简称**定稿**，行号以 `/root/docs_archive/RPG框架项目/怪物模块设计定稿.md` 为准；辅助引用《怪物行动AI设计定稿》= AI 定稿）
...
> 标注约定：**定稿既有**=定稿原文明确；**细化定型**=定稿仅给区间/单例/方向，本表取点定型（如 PV 档默认值 10/75/**300 取定稿样本值**〔低/中档取区间中值，BOSS 档 200-500 中值实为 350、300 为 AI 定稿样本值 AI L236〕；`pv_recover` 枚举在样本 `battle_end` 外补 `none`）——取点可改，裁决边界透明。
...
引用约定：`[Lxx]` = 定稿行号；`[AI Lxx]` = 怪物行动AI设计定稿行号；"派生" = 由定稿语义直接推出的防呆规则（标注推出来源）。
...
| F03 | `tier` | enum | 必填 | `normal`（**细化定型**：定稿仅给枚举，默认取样本 normal [L21]） | `normal` / `elite` / `boss` / `training`（第四档=木桩，仅战斗外 /木桩 可进入） | L21、L338 |
| F04 | `type` | enum | 选填 | 无 | `"dummy"`（标记式，tier 保持 normal 也可）；与 `tier: training` **至少其一**构成木桩 | L339 |
...
| F08 | `weakness` | object | 必填 | 无（空对象） | `types` + `elements` 双维，见 1.3；建议 ≥1 弱点（校验器警告，木桩豁免） | L25、L55-62、L365 |
| F09 | `pv` | number | 必填（普通怪） | 按档默认（见②） | ≥0；档区间仅提示：普通 0-20 / 精英 50-100 / BOSS 200-500；**木桩强制 0** | L26、L101、L237-239、L352 |
| F10 | `pv_recover` | enum | 选填 | `battle_end` | `battle_end` / `none`（战斗结束 PV 是否重置）；**换区恢复一半为运行时规则**（残血换区→PV 恢复一半向下取整），不由本字段表达 | L278、L93-100 |
...
| F14 | `chains` | array | 选填 | `[]` | **顶层连招表（新配置唯一载体）**；节点结构以 AI 定稿 §八为准（本表只登记不展开）；旧 action.json 内嵌 `chain` 读兼容+迁移提示 | L137、L269；AI L248-266 |
...
| F17 | `def_base` | number | 选填（木桩向） | 无 | ≥0；防御基准直读代入 或 映射 stats 体质，**二选一**（同配提示一致性）。**实现定型**：默认直读（缺省 def_base 时映射 stats 体质作兜底）；二选一判定以「是否显式配置 def_base」为准——配置了=直读，未配置=映射；两者同配 → 校验器提示一致性（黄提示不拦截） | L341 |
| F18 | `elem_res` | object | 选填（木桩向） | 无 | 元素 ID ∈ 注册表；值正=减伤 / 负=增伤（相对 formula.json 元素减伤系数） | L342 |
...
|---|---|---|---|---|---|---|---|
| S01 | `hp` | 生命 | number | 必填 | 模板基准 | ≥0；木桩=极大值（≥目标斩杀回合总输出上限） | L40、L340 |
...
| W02 | `weakness.elements` | object | 选填 | 键=元素 ID（**8 元素注册表引用检查**，不硬编码清单；示例 `fire`/`water`）；值=增伤倍率，**×1.3 默认**；双维独立判定、同中叠乘（参考伊甸 ×2×2） | L56、L60、L299 |
| W03 | `resistance`（初始抗性键） | object（键=负面效果 ID） | 选填 | 值 0-100（初始层数/概率门槛）；示例键 `stun`/`shock`/`shadow_seal`，负面效果 ID 键可扩展 | L111、L116 |
| W04 | `resistance.immune` | string[] | 选填 | 完全免疫的负面效果列表（示例 `["poison"]`）；施加前拦截 | L112、L251 |

门禁管线顺序（效果系统联动，只读引用）：免疫判定 → 抗性初始层数 → PV 减半（PV>0 时数值减半、层数照常）→ 破防全量 [L251]。

### 1.4 actions / special_actions / chains —— 行动表、特殊行动、连招载体

**actions 条目**（6 字段，AI 定稿 §一）：

| # | 字段 | 类型 | 必填 | 约束 | 来源 |
|---|---|---|---|---|---|
| A01 | `actions[].action` | string | 必填 | 引用 action.json 的行动 ID，**引用必须存在**（校验器） | L281、L297 |
| A02 | `actions[].probability` | number | **选填** | **默认 0**（漏配=锚点行动，只被链/条件/状态机触发；AI 定稿 L25）；1=入池（参与随机池）；**写其他正值等价 1**（AI 定稿 L51）；数值不参与概率计算 | L138、L145、L281；AI L25、L51 |
| A03 | `actions[].weight` | number | 选填 | ≥0；随机池内归一化权重（例：普攻 50/重击 30/蓄力 20 → 归一化 1/2、3/10、1/5）；**默认值兜底**（AI 定稿 L29）；概率=weight 归一化 | L145、L281；AI L29 |
| A03b | `actions[].condition` | object/string | 选填 | 条件权重修正（如 pv_broken 时 ×2），默认 null（AI 定稿 §一） | AI L41 |
| A03c | `actions[].cooldown` | number | 选填 | 行动冷却回合数，默认 0 | AI L42 |
| A03d | `actions[].hungry` | number | 选填 | 饥饿保底：连续 N 回合未选中则强制选，默认 0=关闭（可配 settings/enemies.json 覆盖） | AI L43 |

**special_actions 条目**（8 字段，AI 定稿 §二；含 trigger 5 参数）：
...
| A10 | `trigger.once` | bool | 选填 | 一次性触发（true=只触发一次；示例 hp_below once:true） | AI L79 |
| A11 | `priority` | number | 选填 | 条件行动优先级：多个条件满足时 priority 降序、同级随机 | AI L26、L83 |
| A12 | `trigger_cooldown` | number | 选填 | 触发冷却（回合数），示例 5 | AI L84 |
| A13 | `max_triggers` | number | 选填 | 最大触发次数，示例 1 | AI L84 |
| A14 | `post_state` | object | 选填 | 触发后进入状态 `{state, turns}`（示例 `{state:"exhausted", turns:1}`） | AI L85 |
| A15 | `chain_ref` | string | 选填 | **连招引用 → 顶层 chains 表条目 id**（连招唯一载体；旧内嵌 chain=历史写法，解析器兼容迁移） | AI L86、L248-266 |

§7.1 面向示例 5 类 ↔ 13 类枚举映射（S3 裁决落地；canonical = AI 定稿 §二 权威名，旧名仅兼容别名）：

| §7.1 语义 | 示例写法（L178-183） | canonical 枚举（权威） | 旧别名（兼容，R-01） | timing 示例 |
|---|---|---|---|---|
| ① 血量低于 xx%（狂暴/残血大招） | `hp_below` | `hp_below` | — | current_turn |
| ② 防护崩溃后（破防反扑） | `pv_broken` | `pv_broken` | `broken` | next_turn |
| ③ 起身后（被击倒/控制恢复） | `get_up` | `get_up` | `revive` | next_turn |
| ④ 进入战斗时（开场技：第一回合、**换区后第一回合**） | `battle_start` | `battle_start` | `enter_phase` | first_turn |
| ⑤ xx 行动后（固定连招锚点） | `after_action` | `after_action` | — | next_turn |

**chains 顶层表**（F14，AI 定稿 §八 [AI L248-266]）：条目 `{id, actions[]}` —— `id`=链唯一标识（**chain_ref 引用目标**，全文件唯一），`actions[]`=节点数组，节点 `{action, chance 0-1（示例 0.8/1.0）, role: chain\|finisher, armor?}` [AI L250-258]；校验：id 非空唯一 + 节点 action 引用存在 + chance 0-1 + role ∈ {chain, finisher}（拦截）；**chain_ref → chains.id 引用存在**（拦截）；链不成环（**细化派生**：成环提示不拦截——环形链=有意的循环连招〔AI L178〕时放行）；旧 action.json 内嵌 chain 读兼容并提示迁移顶层 chains。`condition` 若配置为【工程补白】扩展（AI 定稿节点无 condition 字段）。
...
| # | 字段 | 类型 | 必填 | 默认值 | 约束 | 来源 |
|---|---|---|---|---|---|---|
| D01 | `drops.*[].item` | string | 必填 | — | 物品 ID，**引用存在**（对齐 reward 条目同键） | L212、L301 |
| D02 | `drops.*[].chance` | number | 必填 | — | **概率 0-100**（整数） | L212、L301 |
| D03 | `drops.*[].condition` | string | 选填 | 无 | 条件触发（special 类使用）：`pv_broken`（破防）/ `no_damage`（无伤）/ `after_action:<action_id>`（特定行动后，**细化定型**：定稿仅「特定行动后」语义 [L209]，`after_action:<id>` 语法为细化定型） | L199、L209、L309 |
...
| # | 字段 | 类型 | 必填 | 约束 | 来源 |
|---|---|---|---|---|---|
| L01 | `lore[].unlock` | number | 必填 | **1-100**，条目间**递增**（10/50/100…）；解锁需求=图鉴进度阈值 | L221-227、L302 |
...
- **顶层 18**（F01-F18）；**全量 58** = 顶层 18 + stats 9 + weakness 2 + resistance 2 + actions 条目 6（A01-A03 + A03b/c/d）+ special_actions 条目 12（A04-A15）+ drops 三类 3 + 掉落条 4 + lore 条 2。
- 不展开计数：`chains` 节点结构（按 AI 定稿 §八）、BOSS 编排扩展 `phases`/`zone_change`/`teach_map`/`pressure`（AI 定稿 §八，不在八段范围）。

---

## ② 难度模板（低/中/高）默认值表

映射裁决（S-模板）：**低=普通 normal / 中=精英 elite / 高=BOSS boss**（依据：L42-45 模板乘区按普通/精英/BOSS 叙述；L364"对齐内容包怪物难度模板（普通/精英/BOSS 三档取值）"）。模板数 = **3**。
...
*细化定型：定稿仅给区间（L101），默认取区间中值（低 0-20→10 / 中 50-100→75），BOSS 档默认 300 取 AI 定稿样本值（AI L236，200-500 区间中值实为 350，300 非中值——按样本定型），可改。

默认值落地语义：
- stats 漏配键 → 按所在档模板自动补全 + 校验器提示（防配装负担）[L42、L318]。
- pv 缺省 → 取档默认值（10/75/300）；显式配置仅做区间提示不拦截 [L300]。
- 木桩不套模板（HP 极大、防御/元素抗性自配，可对齐三档取值作参考）[L340、L364]。
...
|---|---|---|---|---|---|
| `chance` | number | 必填 | **0-100**（整数） | 掉落特有（概率化） | L212、L301 |
| `condition` | string | 选填 | 枚举：`pv_broken` / `no_damage` / `after_action:<action_id>`（破防/无伤/特定行动后） | 掉落特有（条件结算，special 类） | L199、L209、L309 |
| `count` | number \| [min,max] | 必填 | 数量或**闭区间 [min,max]**，min≤max、非负整数 | **与 reward 条目同键**（同 schema） | L196、L202、L212 |
| `item` | string | 必填 | 物品 ID 引用存在 | **与 reward 条目同键** | L212、L301 |
...
**判定**：`tier: "training"` 或 `type: "dummy"` **至少其一**命中即按木桩处理 [L330、L338-339]。

| 类别 | 项目 | 规则 | 来源 |
|---|---|---|---|
| 忽略项（配置了也忽略，**黄提示不拦截**） | `drops` | 无掉落：忽略 drops；无金币、无经验、不计击杀类任务进度 | L351、L303 |
| | `lore` | 不入图鉴：无 lore 解锁，不占 codex_state 进度 | L353、L303 |
| | `pv` | **强制 0**（防 debuff 门禁干扰伤害测量）；配置了也忽略+黄提示 | L352、L303 |
| | `actions` / `special_actions` | **不可反击**：木桩不行动，忽略行动表/特殊行动/AI 状态机（配置了也忽略+黄提示） | L350 |
| 可配项 | `stats.hp` | HP 极大（≥ 目标斩杀回合总输出上限，防打死中断测量） | L338 |
| | `def_base` | 防御基准 ≥0：直读代入 或 映射 stats 体质，**二选一**（同配提示一致性） | L341 |
| | `elem_res` | 元素抗性基准：正=减伤 / 负=增伤（相对 formula.json） | L342 |
| | `weakness` | 可选：模拟目标怪弱点，测元素/类型配装；**豁免"每怪 ≥1 弱点"约束** | L341、L365 |
| 运行期 | 战斗规则 | 与普通战斗共用引擎（命中/会心/格挡/双通道/拦截链全量生效）；战败→无损退出（可配 0=按普通战败处理）；/撤退（=/逃跑）中断快照续玩 | L354-356 |
| | 入口 | /木桩 → 档位列表；/木桩 <序号或名称> → 进入；不在地图内，锁定/进入地图流程不适用 | L368-372 |
| | 调整木桩 | /调整木桩 <怪物名> 复制该怪 def_base/elem_res/弱点/抗性（HP 仍极大，不复制攻击/行动）；无参=重置；覆盖存 **dummy_override（settings/世界状态）**，**不落 enemies 文件** | L381-384 |
| 多档 | 档位建模 | 档位=一只木桩（独立 id），作者按需建 N 只（低防脆皮/高防重甲/高元素抗/高敏捷/全抗…）；数值建议对齐三档难度模板 | L357-362 |
| 校验 | 数值 | HP/防御/元素抗性基准**非负**（拦截）；弱点 ≥1 约束**不适用** | L365 |

---

## ⑤ 校验器规则

级别：**拦截**（结构错误，拒绝入库）/ **警告**（黄，不拦截）/ **提示**（信息）。定稿原条 7 条（R1-R7，对应定稿 L297-303 校验 1-7）+ 派生规则 8 条（R8-R15，语义直接推出）。
...
| R6 | 图鉴情报 unlock 1-100 且递增 | 拦截 | L302（校验 6） |
| R7 | 木桩（tier:training / type:dummy）：drops/lore/pv 若配置 → 黄提示"木桩忽略掉落/图鉴/PV"（不拦截）；防御/元素抗性基准非负 | 警告 / 拦截 | L303（校验 7） |
| R8 | 必填与枚举：id/name 非空、id 唯一；tier ∈ {normal,elite,boss,training}；木桩标记至少其一（派生自结构定义） | 拦截 | L19-21、L338-339 |
| R9 | stats 键合法：9 键 ∈ 固定键集、数值≥0；漏配键按难度模板补全并提示（派生自模板语义） | 拦截 / 提示 | L40-45、L42、L318 |
| R10 | 概率语义：probability 选填默认 0（漏配=锚点）、1=入池、其他正值等价 1；weight ≥0（默认兜底）；随机池=probability≥1（等价 1）条目按 weight 归一化。**硬拦仅：类型错误/负数/结构错误**；probability 非 0/1（如 2）→ 按等价 1 容忍（不拦截）；**概率总和为 0 / 空随机池 → 黄提示**（AI 定稿 L29/L51/L282-283；只建议不限制） | 拦截（4 类）/ 黄提示 | AI L25、L29、L51、L282-283；L138、L145、L281 |
...
| R12 | 触发别名规范化：旧别名 broken/revive/enter_phase 接受（兼容旧配置），提示归一为权威名 pv_broken/get_up/battle_start（细化_0 R-01） | 提示 | AI 定稿 §二 vs L298 |
| R13 | 掉落扩展域：condition ∈ {pv_broken, no_damage, after_action:<action_id>}；count 为 number 或 [min,max] 且 min≤max、非负 | 拦截 | L198-199、L212、L309 |
...
| TC-14 | 换区/开场技行为验收（运行期） | 残血换区（/进入 通道）后续战 | 血量保持残血、PV 恢复一半（向下取整）、换区后第一回合触发开场技（battle_start）；玩家离开副本 → 下次满状态重打 |

---

## ⑦ 跨文档登记（实现前必读，2026-08-25 复审新增）
...
| 天然抗性 / 门禁管线 | L106-118、L251 |
| 行动数据库 / 概率语义 | L122-148 |
| 特殊行动 5 类示例 / 13 类枚举 | L152-185、L298 |

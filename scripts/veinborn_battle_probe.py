"""veinborn 实机战斗流程：进入猎场 → 遭遇砾冕 → 四色出牌/部位破坏。"""
import asyncio
import os
import sys
import uuid
...
os.environ["QBotRPG_DB_PATH"] = "/tmp/rpg_veinborn_battle.db"
...
async def main() -> None:
    if os.path.exists("/tmp/rpg_veinborn_battle.db"):
        os.remove("/tmp/rpg_veinborn_battle.db")
    deps = await build_app_deps()
    out: list[str] = []

    async def say(text: str, uid: str = "u_vb2") -> str:
        ev = {
    ...
    text = "\n\n".join(out)
    with open("/tmp/veinborn_battle_flow.txt", "w", encoding="utf-8") as f:
        f.write(text)

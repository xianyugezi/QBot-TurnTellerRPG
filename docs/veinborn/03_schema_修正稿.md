...
## 十四、官方规则校准（2026-09-02 增补，来源：用户提供的《原初觉醒》官方规则资料 5 份）

> 用户在 03 修正稿产出后提供了 Primal 官方规则资料（00-05，已归档 /root/docs_archive/原初觉醒_Primal官方规则/）。本节记录「官方规则 vs veinborn 已拍板方案」的核对结论——全部对位，无返工；另补 3 处规则真相供后续数值/文案对齐。

### 14.1 已拍板方案 × 官方规则 对位确认（全部 ✅）

| veinborn 方案 | 官方规则依据 | 结论 |
|---|---|---|
| 机动牌=跃空=移除困斗 | 官方：🔵机动「移除凶兽板上 1 个困斗标记」（05 §3.1）；「猎人可用机动卡移除 1 个 Struggle 控量」（03 §四） | ✅ 与拍板①完全一致（veinborn 稿没歪） |
| 困斗满→宣泄大招 | 官方：困斗≥3×人数触发 Unleash（05 §2.2）；宣泄后 token 减至 1×人数 | ✅ 拍板②的 enemy_mark min 触发 + mark_remove 回落，语义一致 |
| 格挡/躲避=防御牌回精力 | 官方：防御牌（黄/绿）是消耗判定免伤依据（05 §6.1）；「防御即续航」 | ✅ 精力循环保真 |
| 部位破坏=独立印记 | 官方：红攻击牌「破坏部位」（03 §五），部位破坏可作锻造素材 | ✅ 方向对（引擎无独立 HP，印记近似已是最优） |
| 受威胁 | 官方真相：**不移动→受威胁**（05 §6.1），非 veinborn 稿「未出某色牌→受威胁」 | ⚠️ 见 14.2 修正 |
| 眩晕/易伤 | 官方：眩晕=揭示 2 红打 2×武器伤害、单一来源；易伤=该回合下次受伤翻倍、不可叠加 | ✅ 印记 max_stack:1 表达对位 |
| 10 回合限制 | 官方：第 10 回合结束未击败凶兽→失败 | ✅ 框架回合上限配置可承载 |

### 14.2 三处规则真相（校准 veinborn 设计稿词面偏差）

1. **受威胁 Threatened 真实规则 = 「本回合不移动 → 受威胁」**（05 §6.1：移动阶段花 1 精力移相邻扇区；不移动则受威胁并取威胁标记，受威胁者消耗判定更危险）。veinborn 稿「未出某色牌→受威胁」是误读。
   → **veinborn 修正**：若做受威胁，应表达为「未位移（未出跃空牌）→ 受威胁」。但引擎回合末无「是否出过某技能」审计点，坑 2 依旧成立——**首版仍不做受威胁**，v0.2 若做，方案=机动牌防御值挂「已走位」印记 + 回合末敌方按无印记挂威胁（仍受引擎限制，待实证）。
2. **易伤 Vulnerable 官方口径**：该回合内**下一次**受伤翻倍、不可叠加（04 Q8）。veinborn 的 vulnerable 印记 max_stack:1 + condition 增伤 ×2 → 对位正确（若条件支持「next damage once」则更贴，否则按「本回合有破绽则增伤」近似，首版可接受）。
3. **眩晕 Stun 官方口径**：揭示 2 红按 2×武器伤害打怪（单一来源），非「跳行动」。veinborn 若把撼击做成「跳行动」是**偏离官方**。
   → **veinborn 修正**：stun 印记语义从「跳行动」改为「下 1 次受击 ×2 且无视防御」近似眩晕伤害（marks 只能做层数/条件，无法做「揭示 2 红」），或首版干脆不做眩晕、用「破绽（易伤）」+「倒地窗口」两个增伤表达代替。**首版倾向只做破绽，不做眩晕**（简化 + 不偏离官方）。

### 14.3 对完整 MVP 的数值/文案启示（官方规则速查，供后续批次）

- 凶兽 HP 从 10 起（姿态伤害轨）；姿态 I/II/III 随受伤推进，坚韧值×人数；姿态推进值换姿态、剩余伤害带入
- 困斗阈值 3×人数、宣泄后回落 1×人数；Escalation（牌库空）→ +1 困斗
- 起始套牌 24 张（6 攻/6 机动/6 格挡/6 躲避）+ 1 专精 + 初始手牌 5；行动阶段最多 5 张、结束手牌≥2 得 1 精力
- 移动花 1 精力/回合 1 次；不移动→受威胁
- 怪物行为牌 3 槽 + 反应图标（对应玩家出牌色触发）+ 强化 Boost（花困斗）+ 暴走 Rampage
- 消耗阶段：按本回合防御牌数判定免伤；受威胁者揭 2 取最不利
- 眩晕=2×武器伤害单一来源；易伤=下次受伤翻倍不可叠加；狂战士=本回合每打攻击抽 1
- 战役：11 章、铁匠铺按元素解锁 3 级、6 元素（Fire/Horn/Coral/Crystal/Thunder/Metal）+ 6 材料、草药师药水
- **元素对应**：veinborn 可用框架 8 元素（地水火风雷晶月无）对应 Primal 9 属性生态，砾冕（脊冢兽·土系）弱 wind/lightning 之类按内容定

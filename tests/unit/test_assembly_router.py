...
ALL_REGISTERED = {
    ...
    # 批75 · GM 运维 5 条（G5/G6/G7/G9/G11）：/调试 /测试 /广播 /玩家查询 /解封
    "调试", "测试", "广播", "玩家查询", "解封",
    ...
    # M12.5 温室（2026-09-06 批2路2）
    "温室",
    # M12.5 委托（2026-09-06 批2路3：/委托 委托板指令族）
    "委托",
    # M12.5 品评会（2026-09-06 批3路2：/投稿 /排行榜）
    "投稿", "排行榜",
    # 批41 · 深度打造主入口（图纸/材料/打造主流程；白名单已同步登记）
    "精造",
    # 批62 · 装备淬炼独立指令壳（批57 引擎/状态/配置/校验之上补指令；白名单已同步登记）
    "淬炼",
    # M9 锻造（批C 审查 P0-1 收口 2026-08-30：六指令注册；/确认 状态分派器
    # replace 接管炼金同名，故 /确认 仍在此集合单份）
    "锻造", "图纸", "锻造树", "套装", "客制",
    # 2026-08-31 实机反馈：口语同义词「打造」「铸造」注册为锻造别名
    "打造", "铸造",
    # M10 钓鱼（批8 审查 A3 P0-1 收口 2026-09-01：三指令注册）
    "钓鱼", "鱼讯", "收杆",
    # M11 成就（m11 启动包 §2.1：三指令注册）
    "成就", "成就信息", "称号",
    # M11 PVP（m11 启动包 §2.3：双指令注册）
    "锁定玩家", "攻击玩家",
    # 2026-09-05 职业真注册（原 stub 转正）：职业/职业列表/转职/职业详情
    "职业", "职业列表", "转职", "职业详情",
    # M12.5 强化（2026-09-06 指令缺口补全批1路1：/强化 真注册 + /强化信息 /强化保护）
    "强化", "强化信息", "强化保护",
    # M12.5 木桩（2026-09-06 批1路2：/木桩 /调整木桩）
    "木桩", "调整木桩",
    # M12.5 查看目标（2026-09-06 批1路3：战斗内只读目标面板）
    "查看目标",
    # M12.5 赠送（2026-09-06 批2路1：/赠送 原子转移）
    "赠送",
